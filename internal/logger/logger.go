...
func SetupLogger() {
	...
	if viper.GetBool(config.LogToFile) {
		...
		var err error
		logFile, err = os.OpenFile(filePath, flags, 0640)
		if err != nil {
			fmt.Println()
...
func GetPanicFileWriter() io.Writer {
	if panicLogFile != nil {
		return panicLogFile
	}

	f, err := os.OpenFile("./panic.log", os.O_CREATE|os.O_WRONLY|os.O_APPEND, 0600)
	if err != nil {
		return io.Discard
	}
...
func write(coloredLevel, plainLevel, s string) {
	if consoleLog != nil {
		consoleLog.Printf("%s %s %s", ts(), coloredLevel, s)
	}
	if fileLog != nil {
		fileLog.Printf("%s %s %s", ts(), plainLevel, s)
	}
	if jsonFileLog != nil {
		b, _ := json.Marshal(jsonEntry{Time: ts(), Level: plainLevel, Message: s})
		jsonFileLog.Println(string(b))
	}
}
...
func Close() error {
	...
	if panicLogFile != nil {
		if err := panicLogFile.Close(); err != nil {
			errs = append(errs, fmt.Errorf("close panic log file: %w", err))
		}
		panicLogFile = nil
	}
